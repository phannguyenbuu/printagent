...
def create_app() -> Flask:
    ...
    def public_crm_printers() -> Any:
        ...
        with session_factory() as session:
            # 1. Get all printers for this lead
            printers = session.execute(
                select(Printer).where(Printer.lead == lead).order_by(Printer.lan_uid, Printer.ip)
            ).scalars().all()

            if not printers:
                return jsonify({"ok": True, "printers": []})

            # 2. To optimize, get latest counter and status for all involved IPs/LANs
            # We can use a similar logic to dashboard_summary
            lan_uids = {p.lan_uid for p in printers}
            
            # Latest Counters
            latest_counters_stmt = select(CounterInfor).where(
                CounterInfor.lead == lead,
                CounterInfor.lan_uid.in_(list(lan_uids))
            ).order_by(CounterInfor.timestamp.desc())
            
            # Since we can't easily do "distinct on" in all SQL dialects via ORM easily without specialized syntax, 
            # and the dataset per lead shouldn't be massive, we'll fetch and filter in memory or use a subquery approach.
            # However, for simplicity and performance on typical datasets:
            all_counters = session.execute(latest_counters_stmt).scalars().all()
            latest_counter_map: dict[tuple[str, str], CounterInfor] = {}
            for c in all_counters:
                key = (c.lan_uid, c.ip)
                if key not in latest_counter_map:
                    latest_counter_map[key] = c

            # Latest Status
            latest_status_stmt = select(StatusInfor).where(
                StatusInfor.lead == lead,
                StatusInfor.lan_uid.in_(list(lan_uids))
            ).order_by(StatusInfor.timestamp.desc())
            
            all_status = session.execute(latest_status_stmt).scalars().all()
            latest_status_map: dict[tuple[str, str], StatusInfor] = {}
            for s in all_status:
                key = (s.lan_uid, s.ip)
                if key not in latest_status_map:
                    latest_status_map[key] = s

            # 3. Get Agent context (hostname etc)
            agents = session.execute(
                select(AgentNode).where(AgentNode.lead == lead, AgentNode.lan_uid.in_(list(lan_uids)))
            ).scalars().all()
            agent_map = {a.lan_uid: a for a in agents}
            
            lan_sites = session.execute(
                select(LanSite).where(LanSite.lead == lead, LanSite.lan_uid.in_(list(lan_uids)))
            ).scalars().all()
            lan_map = {ls.lan_uid: ls for ls in lan_sites}

            # 4. Assemble result
            results = []
            for p in printers:
                key = (p.lan_uid, p.ip)
                c = latest_counter_map.get(key)
                s = latest_status_map.get(key)
                a = agent_map.get(p.lan_uid)
                ls = lan_map.get(p.lan_uid)

                results.append({
                    "lan_uid": p.lan_uid,
                    "agent_uid": p.agent_uid,
                    "lan_name": ls.lan_name if ls else "",
                    "hostname": a.hostname if a else "",
                    "printer_name": p.printer_name,
                    "ip": p.ip,
                    "mac": p.mac_address,
                    "counter": c.total if c else None,
                    "status": s.system_status if s else ("Online" if p.is_online else "Offline"),
                    "alerts": s.printer_alerts if s else "",
                    "toner": s.toner_black if s else "Unknown",
                    "last_seen_at": p.updated_at.isoformat() if p.updated_at else None
                })

            return jsonify({"ok": True, "printers": results})

...
export default function App() {
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          await loadScene(true, true, true, false, active);
          return;
        }
      } catch {}
      await loadScene(true, true, true, false, "convoi.svg");
    })();
  ...
  const sourceOutPath = (name) => {
    ...
    return safe === "convoi.svg" ? "/out/convoi.svg" : `/out/sources/${encodeURIComponent(safe)}`;
  };

  const sourceQuery = (name) => `source=${encodeURIComponent(name || selectedSource || "convoi.svg")}`;
  ...
  const loadScene = async (
    ...
    try {
      ...
      if (updateZone) {
        ...
        try {
          const clickRes = await fetch(`/api/source_zone_click?${sq}`);
          if (clickRes.ok) {
...
<html>
  ...
  const getPackedSourceForCompute = async (sourceName = selectedSource) => {
    ...
    const res = await fetch(`/api/scene?${sourceQuery(sourceName)}&force_compute=0`);
    if (!res.ok) {
      ...
      <div className="content">
        <div className="column-left">
          <div className="panel toolbar pack-toolbar-right">
            <button onClick={() => loadScene(true, true, true, true, selectedSource)}>Load</button>
            ...
            <label className="mini-input">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  loadScene(true, true, true, false, next);
                }}
              >

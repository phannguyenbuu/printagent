...
def api_state():
    ...
    if source_name == DEFAULT_SOURCE_NAME:
        LEGACY_STATE_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def api_get_state():
    ...
    if source_name == DEFAULT_SOURCE_NAME and LEGACY_STATE_JSON.exists():
        return jsonify(_legacy_fallback_json(LEGACY_STATE_JSON))
    return jsonify({})
...
def api_pack_from_scene():
    ...
    print(f"[pack_from_scene] zone_polys built={len(zone_polys)} zone_order={len(zone_order)}")

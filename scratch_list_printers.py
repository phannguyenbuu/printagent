...
cursor = conn.cursor()
cursor.execute("SELECT id, printer_name, ip, mac_address, agent_uid, lan_uid, is_online, updated_at FROM \\\"Printer\\\" WHERE ip = '192.168.1.226'")
rows = cursor.fetchall()
print("Found", len(rows), "printers:")
for r in rows:
    print(f"ID: {r[0]} | Name: {r[1]} | IP: {r[2]} | MAC: {r[3]} | Agent: {r[4]} | LAN: {r[5]} | Online: {r[6]} | Updated: {r[7]}")
conn.close()
"""

sftp = ssh.open_sftp()
temp_remote_file = "/tmp/list_printers_temp.py"
